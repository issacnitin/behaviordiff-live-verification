    ...
    internal sealed class AnthropicExplainer : IDisposable
    {
            ...
            IReadOnlyList<string> citationCorpus)
        {
            ...
            return "Given this evidence, explain in plain language why the unedited member's behavior changed "
                ...
                + "shows CustomerTier was removed from a cache key and a Standard lookup receiving a price "
                + "previously cached by a Gold lookup, explicitly explain that the coarser key allows one tier's "
                + "price to be served to another tier. "
