    ...
    internal sealed class AnthropicExplainer : IDisposable
    {
            ...
            IReadOnlyList<string> citationCorpus)
        {
            ...
            return "Given this evidence, explain in plain language why the unedited member's behavior changed "
                ...
                + "shows OrderBy was replaced with List.Sort and equal-priority rules selecting a different "
                + "discount, explicitly explain that OrderBy preserves declaration order while List.Sort is not "
                + "stable, so tied rules can select a different first match. "
